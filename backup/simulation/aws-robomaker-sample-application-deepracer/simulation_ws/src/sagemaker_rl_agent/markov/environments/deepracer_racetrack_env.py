...
class DeepRacerRacetrackEnv(gym.Env):
    ...
    def __init__(self):
        ...
        if node_type == SIMULATION_WORKER:
            ...
            self.reverse_dir = os.environ.get("REVERSE_DIR", False)
            ...
            self.change_start = rospy.get_param('CHANGE_START_POSITION', (self.job_type != TRAINING_JOB))
            self.alternate_dir = rospy.get_param('ALTERNATE_DRIVING_DIRECTION', (self.job_type != TRAINING_JOB))
    ...
    def set_next_state(self):
        ...
        observation_new = np_image
        observation_new[:, :, 0] = np_image[:, :, rnd_rgb[0]]
        observation_new[:, :, 1] = np_image[:, :, rnd_rgb[1]]
        observation_new[:, :, 2] = np_image[:, :, rnd_rgb[2]]
        #observation_new = observation_new.astype(np.uint8)
        
        import sys
        #sys.path.insert(1, "/app/robomaker-deepracer/simulation_ws/build/sagemaker_rl_agent/build/lib/markov/environments/")
        #sys.path.insert(1, "/app/robomaker-deepracer/simulation_ws/src/sagemaker_rl_agent/markov/environments/")

        #observation_style = self.feed_transform(np_image, "/app/robomaker-deepracer/simulation_ws/src/sagemaker_rl_agent/markov/environments/checkpoints/udnie")
        #observation_style = np.clip(observation_style, 0, 255).astype(np.uint8)
        
        #im = PIL.Image.fromarray(numpy.uint8(observation_new))
    ...
    #### ALEX INSERTED #####
    def feed_transform(self, img_in, checkpoint_dir):
        print("STAGE 2")
        import tensorflow as tf
        
        with tf.name_scope("dr_style_trasfer"):
            ...
            #with tf.Session(config=soft_config) as sess:
                ...
                if os.path.isdir(checkpoint_dir):
                    #if not checkpoint_dir in self.ckpt:
                    #    self.ckpt[checkpoint_dir] = tf.train.get_checkpoint_state(checkpoint_dir)
                    #ckpt = self.ckpt[checkpoint_dir]
                ...
                img = np.asarray([img_in]).astype(np.float32)
                #start_tic = time.time()
    ...
    def step(self, action):
        ...
        try:
            ...
            avg_perc = int(os.environ.get("AVERAGE_PERCENTAGE", 0))
            median_perc = int(os.environ.get("MEDIAN_PERCENTAGE", 0))
        except Exception as ex:
            print("No training round variable found")
        
        if self.job_type == TRAINING_JOB:            
            if median_perc < 100:
                self.steering_angle += 0.05 * np.random.normal(0, 1.0, 1)
                self.speed += 0.4 * np.random.normal(0, 1.0, 1)
            elif median_perc == 100 and avg_perc < 8200:
                self.steering_angle += 0.1 * np.random.normal(0, 1.0, 1)
                self.speed += 0.4 * np.random.normal(0, 1.0, 1)
            elif median_perc == 100 and avg_perc < 9200:
                self.steering_angle += 0.3 * np.random.normal(0, 1.0, 1)
                self.speed += 0.4 * np.random.normal(0, 1.0, 1)
            else:
                self.steering_angle += 0.5 * np.random.normal(0, 1.0, 1)
                self.speed += 0.4 * np.random.normal(0, 1.0, 1)
        else:
            evaluation_test = False
            try:
                evaluation_test = os.environ.get("EVALUATION_TEST", False)
            except Exception as ex:
                pass
            
            if evaluation_test == False:
                #self.steering_angle += 0.91 * np.random.normal(0, 1.0, 1)
                if median_perc < 100:
                    self.speed *= 1.1
                elif median_perc == 100 and avg_perc < 9400:
                    self.speed *= 1.7
                    self.steering_angle += 0.05 * np.random.normal(0, 1.0, 1)
                else:
                    self.speed *= 2.0
                    self.steering_angle += 0.1 * np.random.normal(0, 1.0, 1)
                #self.speed += 0.2 * np.random.normal(0, 1.0, 1)
            else:
                pass
    ...
    def __call__(self, img, name='transfer', is_reuse=False):
        ...
        with tf.name_scope("dr_style_trasfer"):
            with tf.variable_scope(name, reuse=is_reuse):
                ...
                resid = self.n_res_blocks(conv3, num_blocks=5, name='res_blocks', is_print=False)
        ...
        with tf.name_scope("dr_style_trasfer"):
            with tf.variable_scope(name):
                ...
                input_ = tf_utils.conv2d(input_, output_dim=num_filters, k_h=filter_size, k_w=filter_size,
                                         d_h=strides, d_w=strides, padding='VALID', is_print=False)
